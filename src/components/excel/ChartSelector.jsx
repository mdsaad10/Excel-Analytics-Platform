...
const ChartSelector = ({ data, activeSheet }) => {
  ...
  // Filter potential X-axis columns (string or date types)
  ...
  useEffect(() => {
    ...
    setChartData(preparedData.slice(0, 50)); // Limit to 50 data points for performance
  ...
  const renderChart = () => {
    ...
    switch(chartType) {
      ...
      case 'pie':
        // For pie chart, we use the first data point for simplicity
  ...
  const handleExportPNG = () => {
    if (chartRef.current && chartData.length > 0) {
      ...
      exportChartAsPNG('chart-container', fileName);
    }
  };
  ...
  const handleExportPDF = () => {
    if (chartRef.current && chartData.length > 0) {
      ...
      exportChartAsPDF('chart-container', fileName, { title });
    }
  };
          ...
          {chartData.length > 0 && (
            <div className="flex space-x-2">
              ...
              <button
                ...
                className="px-3 py-1 text-sm bg-red-500 text-white rounded hover:bg-red-600 transition-colors flex items-center"
                title="Export as PDF"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
